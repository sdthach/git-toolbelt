...
> **Fork gotcha:** `gh` resolves a fork's base repo to its **parent**, so a bare `gh pr create` in this repo targets `nvie/git-toolbelt`, not us. Every workflow step that shells out to `gh` therefore sets `GH_REPO: ${{ github.repository }}`. Keep that in mind when adding new `gh` calls — and locally, pass `--repo sdthach/git-toolbelt` or run `gh repo set-default sdthach/git-toolbelt` once.
