...
Based on upstream `nvie/git-toolbelt` v1.12.0.

- **Distribution moved from Homebrew to [mise](https://mise.jdx.dev).** Install with
  `mise use -g github:sdthach/git-toolbelt` — one platform-independent build for
  macOS, Linux, and WSL, and no tool options needed on the user's side
- **Version scheme is now plain `vX.Y.Z`**, owned by the fork rather than derived from
  upstream's numbering. The old `-fork.N` suffix is retired: it sorted as a pre-release
  in mise and forced an explicit `version` pin to work around Homebrew's mis-parse.
  `release.yml` rejects any tag that isn't `vMAJOR.MINOR.PATCH`
- New `scripts/build-dist.sh` assembles the release tarball, flattening the root `git-*`
  scripts and `portmanteaus/` into the single `bin/` directory that mise discovers with
  no `bin_path` option. The archive is byte-reproducible for a given commit. The repo
  layout is unchanged, so upstream merges stay conflict-free
- New `scripts/smoke-test.sh` builds that tarball and exercises it the way an install
  does — catching a broken archive, a lost exec bit, or a missing shebang in CI rather
  than on a user's machine
- Releases now ship a `.sha256` asset that mise verifies automatically, plus GitHub
  build-provenance attestations
- New `mise.toml` defines the dev environment (repo `PATH`, `shellcheck`, `yamllint`)
  and the `lint` / `build` / `smoke` / `ci` tasks. `.envrc` delegates to it instead of
  keeping a second copy of the `PATH` list
- CI: `lint.yml` becomes `ci.yml` and runs `mise run ci` — the same command as local, so
  the two can't drift. `yamllint` is now actually run (`.yamllint` had been configured
  but never wired up), and the release tarball is smoke-tested on every push
- Removed: `packaging/git-toolbelt.rb`, the `homebrew-tap` bump job, and the `TAP_TOKEN`
  secret requirement. Releases now need no secrets and no second repository
